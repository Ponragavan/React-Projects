    ...
    <div className="w-[35rem] mt-16">
      <header className="pb-4 mb-4 border-b-2 border-stone-300">
        <div className="flex items-center justify-between">
          ...
          <button onClick={props.onDelete} className="text-stone-600 hover:text-stone-950">Delete</button>
        </div>
        <p className="mb-4 text-stone-400">{formattedDate}</p>
        <p className="text-stone-600 whitespace-pre-wrap">{props.project.description}</p>
      </header>
      <Task onAddTask={props.onAddTask} onDeleteTask={props.onDeleteTask} tasks={props.tasks}/>
    </div>
  );
};
